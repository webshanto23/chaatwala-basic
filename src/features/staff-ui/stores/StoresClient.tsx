...
const CreateStoreModal = dynamic(() => import("@/components/staff/create-store-modal").then(m => m.CreateStoreModal), { ssr: false });
const EditStoreModal = dynamic(() => import("@/components/staff/edit-store-modal").then(m => m.EditStoreModal), { ssr: false });
...
export function StoresClient({ initialStores, initialManagers }: { initialStores: Store[]; initialManagers: Manager[] }) {
  ...
  return (
    <div className="space-y-6">
            ...
            <Card key={store.id} className="border-border/70 overflow-hidden">
              ...
              <CardContent className="space-y-1 text-sm text-muted-foreground">
                ...
                <div>
                  ...
                  {store.manager ? (store.manager.name ?? store.manager.username ?? "Unnamed staff") : "None added Yet"}
                </div>
              </CardContent>
              {canUpdate && (
      ...
      {open && <CreateStoreModal onClose={() => setOpen(false)} onCreated={handleCreated} />}
