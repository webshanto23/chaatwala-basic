      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
            ...
            <Button onClick={loadLogs} disabled={loading}>{loading ? "Loading..." : "Apply"}</Button>
          </div>
        </CardContent>
      </Card>
          ...
          {logs.length === 0 ? (<p className="text-sm text-muted-foreground">No audit logs found.</p>) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b"><th className="pb-2 text-left font-medium">When</th><th className="pb-2 text-left font-medium">User</th><th className="pb-2 text-left font-medium">Action</th><th className="pb-2 text-left font-medium">Entity</th><th className="pb-2 text-left font-medium">Details</th></tr></thead>
                <tbody>
                  {logs.map((log) => (
                    <tr key={log.id} className="border-b last:border-0">
                      <td className="py-3 whitespace-nowrap">{new Date(log.createdAt).toLocaleString()}</td>
                      <td className="py-3"><div className="font-medium">{log.user?.name ?? "Unknown"}</div><div className="text-muted-foreground">{log.user?.email ?? ""}</div></td>
                      <td className="py-3"><span className="inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium">{log.action}</span></td>
                      <td className="py-3">{log.entity}{log.entityId ? ` #${log.entityId}` : ""}</td>
                      <td className="py-3">{log.metadata ? (<pre className="max-w-xs overflow-x-auto rounded-md bg-muted p-2 text-xs">{JSON.stringify(log.metadata, null, 2)}</pre>) : (<span className="text-muted-foreground">-</span>)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
