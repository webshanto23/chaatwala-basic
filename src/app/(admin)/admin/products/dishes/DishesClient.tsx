...
export function DishesClient({ initialDishes }: { initialDishes: DishRowFull[] }) {
  ...
  const handleUpdated = (dish: DishRowFull) => {
    setDishes((prev) => prev.map((d) => d.id === dish.id ? { id: dish.id, name: dish.name, price: `$${Number(dish.price).toFixed(2)}`, tag: dish.tag ?? "-", available: dish.isAvailable ? "Yes" : "No" } : d));
    setFullDishes((prev) => prev.map((d) => (d.id === dish.id ? dish : d)));
  };

  const handleEdit = (row: Record<string, unknown>) => {
    const full = fullDishes.find((d) => d.id === row.id);
    if (full) { setSelectedDish(full); setEditOpen(true); }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Input placeholder="Search dishes..." value={query} onChange={(e) => setQuery(e.target.value)} className="max-w-xs" data-testid="admin-search" />
          {canCreateDish && (<Button onClick={() => setOpen(true)}><Plus className="h-4 w-4" />New Dish</Button>)}
        </div>
      </div>
